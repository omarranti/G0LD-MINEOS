...
- **Reuse confidence:** adapt-the-shape (`ProgressBar` is drop-in; the page, card, form and mutations are wired to Loft's `lib/repo`, `lib/database` types, `lib/neon`, and the project's Tailwind token classes, so they need rehoming)
