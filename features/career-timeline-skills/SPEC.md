...
- **Reuse confidence:** adapt-the-shape (the two data models, the timeline sort, the proficiency-dot pattern and the AI-grounding hook are the value; every file is wired to Loft's `lib/neon`, `lib/database`, `lib/repo`, `lib/insights` and Tailwind design tokens, so they need rehoming)
...
- `getInsight` / `InsightReveal` are Loft's AI-insight overlay; pass `undefined`
