...
import { getSql, NATE_SENTINEL_USER_ID } from "@/lib/neon";
import type {
  CareerCategory,
  MutationState,
  SkillCategory
...
export async function createCareerEvent(
  ...
  try {
    await sql`
      ...
      values
        (${NATE_SENTINEL_USER_ID}, ${title}, ${description}, ${date}, ${category}, ${tags})
    `;
  } catch (err) {
...
export async function createSkill(
  ...
  try {
    await sql`
      ...
      values
        (${NATE_SENTINEL_USER_ID}, ${name}, ${category}, ${proficiency}, ${isTarget})
    `;
  } catch (err) {
    return fail((err as Error).message);
  }
