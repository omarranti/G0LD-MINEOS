...
- **Reuse confidence:** adapt-the-shape (the form / action / list mechanism is drop-in, but it carries Loft couplings: a sentinel single-user id, a `getSql()`-or-demo fallback, and a separate insights layer the list page calls)
...
- **Insights side-channel.** Each card is passed `insight={getInsight("journal", e.id)}`.
  In Loft this is a hand-written lookup table keyed by entry id, NOT generated from
...
- `linked_goal_id` / `linked_contact_id` exist in the schema (entries can be linked
  to a goal or contact elsewhere in Loft) but this capture form never sets them.
- `user_id` is always the sentinel id `DEMO_SENTINEL_USER_ID`
  ...
  Loft is single-user with no auth provisioned yet.

## Key decisions & gotchas
- **Entries do NOT feed the AI / insights layer (yet).** The page calls
  `getInsight("journal", e.id)`, which in Loft is a hand-authored lookup table keyed
